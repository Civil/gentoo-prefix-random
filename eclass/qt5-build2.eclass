...
# @FUNCTION: qt5-build2_src_unpack
# @DESCRIPTION:
# Unpacks the sources.
qt5-build2_src_unpack() {
	...
	if [[ $(gcc-major-version) -lt 4 ]] || \
	   [[ $(gcc-major-version) -eq 4 && $(gcc-minor-version) -lt ${min_gcc4_minor_version} ]]; then
		if [[ ${QT5_MINOR_VERSION} -ge 6 ]]; then
			...
			eerror "You have GCC $(gcc-major-version).$(gcc-minor-version)"
...
qt5-build2_src_prepare() {
    ...
    if use aqua; then
	    sed -i -E 's/^(\s*sysrooted).*/\1 = ""/' "${S}/mkspecs/features/mac/sdk.prf"
	    sed -i 's/macSDKify()$/macSDKify() {\nreturn\n}\ndummy_func()/' configure
	fi
...
# @FUNCTION: qt5-build2_src_compile
# @DESCRIPTION:
# Runs emake in the target directories.
qt5-build2_src_compile() {
	qt5_foreach_target_subdir emake
}

# @FUNCTION: qt5-build2_src_test
# @DESCRIPTION:
...
# @FUNCTION: qt5-build2_src_install
# @DESCRIPTION:
# Runs emake install in the target directories.
qt5-build2_src_install() {
...
# of this function must happen before calling qt5-build2_src_configure.
qt_use_compile_test() {
	[[ $# -ge 1 ]] || die "${FUNCNAME}() requires at least one argument"
...
qt5_base_configure() {
	...
	local conf=(
		...
		$(use aqua && echo " -no-framework --pkg-config=yes -qpa cocoa ")
